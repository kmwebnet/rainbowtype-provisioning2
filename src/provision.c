...
// write device serial and verify
    ...
    status = atcab_write_bytes_zone(ATCA_ZONE_DATA, 13 , 0 , (uint8_t *)serialstr , 20 );
    ...
    status = atcab_read_bytes_zone(ATCA_ZONE_DATA, 13 , 0 , (uint8_t *)verifyresult2 , 20 );
...
vTaskDelay(10 / portTICK_PERIOD_MS);
    ...
    if (ATCA_SUCCESS != atcab_write_enc(8 , 4 , data ,iokeyrandom ,  6 , num_in)) {
   	printf("write data to slot8 failed");
    }
